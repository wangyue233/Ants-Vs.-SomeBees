...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place = self.place
        search_light = 0
        # assuming that the Hive is the end of the game field
        while place.name != 'Hive':
            target_bee = random_or_none(place.bees)
            # if the target bee exists and it's within search parameters
            if target_bee and search_light >= self.min_range and search_light <= self.max_range:
                return target_bee
            else:
                # jump into enterance => next place
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        def double_dmg(place):
            if place.ant and not isinstance(place.ant, QueenAnt) and not place.ant.doubled:
                place.ant.damage *= 2
                place.ant.doubled = True

            if isinstance(place.ant, BodyguardAnt) and place.ant.ant and not place.ant.ant.doubled:
